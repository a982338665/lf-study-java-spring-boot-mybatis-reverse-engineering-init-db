...
//        HttpServletResponse response = HttpServletUtil.getHttpServletObject(HttpServletResponse.class);
    ...
    public List<SMenu> getMenu(Long id) {
        //TODO 暂时不做权限分配
        List<SMenu> list=sUserDao.getMenu();
        return list;
    }

    @Transactional
    public Object login2() throws  Exception {
        for (int i=0;i<5;i++){
//            try {
                sUserService.get();
//            }catch (Exception e){
...
//            }
    ...
    public void get()  throws  Exception{
        try {
            ...
            tongJiService.select();
            pers.li.aseckill.vo.testVo userById = sUserDao.getTestById(testVo.getId());

            System.err.println(userById.toString());
            int x =1/0;
        }catch (Exception e){
